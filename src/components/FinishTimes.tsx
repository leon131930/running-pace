  ...
  return <div className="space-y-4 md:space-y-6">
      ...
      <div className="space-y-3">
        <Label htmlFor="custom-distance" className="text-lg font-semibold text-white">
          Or add your custom distance:
        </Label>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Input id="custom-distance" type="number" value={customDistance} onChange={e => setCustomDistance(e.target.value)} placeholder="0" className="w-24 bg-white/5 border-blue-300/20 text-white placeholder:text-white/50" min="0" step="0.1" />
            <span className="text-blue-200">{unit}</span>
          </div>
          {isValidCustomDistance && <div className="bg-white/5 backdrop-blur-sm rounded-lg p-3 border border-blue-300/10">
